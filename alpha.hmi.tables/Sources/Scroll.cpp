	...
	namespace Hmi {
		namespace Tables {
			namespace Scrolls {
				...
				{
					m_qContentSize = qSize; // берем размеры таблицы
				...
				{
					//auto rc = qSize; // QRectF внешний ФРЕЙМ
					...
					if (rc.width() >= m_qContentSize.width() && rc.height() >= m_qContentSize.height())
						return rc;     // если рамка больше таблицы, выходим

					auto rcHScroll = m_pHScroll->rect();
					auto rcVScroll = m_pVScroll->rect();  // взяли прямоугольники самих скроллов 

					if (rc.width() < m_qContentSize.width())
					{
						// Нужен горизонтальный скролл
						m_pHScroll->setVisible(true);
						rc.adjust(0.0, 0.0, 0.0, -rcHScroll.height()); // начало координат остались, ширина тоже, высота уменьшилась на высоту горизонтального скролла 

						if (rc.height() < m_qContentSize.height())
						{
							// Горизонтальный скролл слишком высокий - нужны оба, или в принципе изначально фрейм был меньше с обоих осей
							m_pVScroll->setVisible(true);
							m_pCorner->setVisible(true);
							rc.adjust(0.0, 0.0, -rcVScroll.width(), 0.0);  // если оба нужны, уменьшаем фрейм еще и в ширь и рисуем уголок со вторым скроллом
						}
					}
				...
				void CQtGraphicsFrame::DrawScrolls(QRectF &rc){
					...
					// отрисовка в нужных местах (где начало то бишь , X)
					m_pHScroll->setPos(0.0, rc.height());
					m_pVScroll->setPos(rc.width(), 0.0);
					m_pCorner->setPos(rc.width(), rc.height());

					// Подгоняем размеры, вторая координата, диагональ противоположная 
					...
					auto pHScroll = qobject_cast<QScrollBar *> (m_pHScroll->widget());  // конвертим Qуказатель обратно в скроллбар
					auto pVScroll = qobject_cast<QScrollBar *> (m_pVScroll->widget());

					// Настраиваем ползунок
					pHScroll->setMaximum((m_qContentSize.width() > rc.width()) ? m_qContentSize.width() - rc.width() : 0); // разница между фреймом и таблицей, на сколько пикселей можно будет продвинуть скроллом
